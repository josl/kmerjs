...
function findMatchesMongoAggregation(kmerMap, url, collection) {
    kmerSchema.index({reads: 1});
    kmerSchema.index({sequence: 1});
    let kmerDB = mongoose.model(collection, kmerSchema, collection);
    ...
    // Query to return matched templates (N_hits)
    let queryMatches = kmerDB
        .aggregate([{ $match: query }])
        .group({
            _id: { sequence: '$sequence', read: '$reads',
                   lengths: "$lengths", ulenght: "$ulenght",
                   species: "$species"},
            uScore: { $sum: 1}
        });
    let nHits = kmerDB
        ...
        .project(
            {
                sequence: 1,
                read_counts: { $sum: "$read_counts_array"}
            }
        )
        .group({
            _id: null,
            count: { $sum: "$read_counts" },
        });
    return Promise.all([
        nHits.exec(),
        queryMatches.exec()
    ...
    .then(([hits, matches]) => {
        let nHits = hits[0].count;
        console.log('Hits found: ', nHits, matches);
        for (let temp of matches){
            ...
            templates.set(sequence, {
                tScore: tCount,
                uScore: uCount,
                lengths: temp._id.lengths,
                ulength: temp._id.ulength,
                species: temp._id.species
            });
        }
...
function findMatchesMongoTest(kmerMap, url, collection, limit=0) {
    kmerSchema.index({reads: 1});
    kmerSchema.index({sequence: 1});
    let kmerDB = mongoose.model(collection, kmerSchema, collection);
    let query = {
    ...
    .then(([matches, summary]) => {
        console.log('Hits found: ', matches);
...
function winnerScoringRecursive(summary, kmerMap, kmerObject) {
    ...
    function recursiveAction() {
        return findMatchesMongoAggregation(kmerMap, kmerObject.db.url, kmerObject.collection, 1)
