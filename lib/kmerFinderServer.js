...
function matchSummary(kmerObject, sequence, match, results, summary){
    ...
    if (match.uScore > minScore){
        ...
        if (allow >= 0) {
            ...
            let expected = new BN(results.hits).times(match.ulength).dividedBy(summary.uniqueLens);
            return new Map([
                ['template', sequence],
                ['score', match.uScore],
                ...
                ['frac-q', fracQ.round(2).toNumber()],
                ['frac-d', fracD.round(2).toNumber()],
                ['coverage', new BN(match.tScore).dividedBy(match.lengths).round(2).toNumber()],
