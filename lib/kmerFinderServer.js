    ...
    if (match.uScore > minScore){
        ...
        if (allow >= 0) {
            let fracQ = new BN(100).times(2).times(match.uScore).dividedBy(
                new BN(Object.keys(kmerObject.kmerMap).length).plus(etta)
            );
            ...
            let totFracQ = new BN(100)
                .times(2)
                .times(kmerObject.firstMatches[sequence].uScore)
                .dividedBy(
                    new BN(Object.keys(kmerObject.kmerMap).length).plus(etta)
                );
            ...
            let totFracCov = new BN(kmerObject.firstMatches[sequence].tScore)
...
function winnerScoring(kmerObject, kmerMap) {
    ...
    function findWinner(results) {
        ...
        if (winner && kmerObject.evalue.cmp(winner['probability']) >= 0){
    ...
    function removeWinnerKmers(matchReads){
        ...
        if (matchReads){
            ...
            for (var kmer in kmerMap){
                if (matchReads.indexOf(kmer) !== -1){
                    newKmers[kmer] = kmerMap[kmer];
                }
            }
    ...
    var loop = function(kmerMap) {
        if (!(notFound && hitCounter < kmerObject.maxHits)){
            ...
            kmerMap = removeWinnerKmers(winnerKmers);
            return loop(kmerMap);
        }
    };
    return {
