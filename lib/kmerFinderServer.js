                ...
                // }
                 for (let hit of hits){
                    ...
                    hit.filteredKmers.forEach(function (read) {
                        ...
                        // let template = templates[hit.template];
                        let kmerCoverage = kmerMap.get(read);
                        // let kmerCoverage = kmerMap[read];
                        if (template !== undefined){
                            template.tScore += kmerCoverage;
                            template.uScore += 1;
                        }else {
                            // templates[hit.template]= {
                            //     tScore: kmerCoverage,
                            //     uScore: 1,
                            //     lengths: hit.lengths,
                            //     ulength: hit.ulengths,
                            //     species: hit.species,
                            //     kmers: hit.filteredKmers
...
function matchSummary(kmerObject, sequence, match, results, summary){
    let minScore = 0;
    let kmerQuerySize = kmerObject.kmerMap.size;
    let firstMatch = kmerObject.firstMatches.get(sequence);
    if (match.uScore > minScore){
        ...
        if (allow >= 0) {
            ...
            let totFracCov = new BN(firstMatch.tScore)
...
function winnerScoring(kmerObject, kmerMap) {
    ...
    function findWinner(results) {
        ...
        // let templates = _.pairs(results.templates).sort(sortKmerMatches);
        if (hitCounter === 0) {
        ...
        let winner = matchSummary(
            ...
            templates[0][1], results, kmerObject.summary
        );
        ...
        if (winner && kmerObject.evalue.cmp(winner.get('probability')) >= 0){
            ...
            return templates[0][1].kmers;
            // return results.templates[winner['template']].kmers;
        }else {
    ...
    function getMatches (kmerObject, kmerQueryMap) {
        ...
        //     for (let queryKmer in kmerQueryMap) {
        //         let kmerCoverage = kmerQueryMap[queryKmer];
        //         if (hit.kmers.indexOf(queryKmer) !== -1) {
        //             let template = templates[sequence];
        //             if (template !== undefined){
        //                 template.tScore += kmerCoverage;
        //                 template.uScore += 1;
        //                 template.kmers.push(queryKmer);
        //             }else {
        //                 templates[sequence] = {
        //                     tScore: kmerCoverage,
        //                     uScore: 1,
        ...
        //         nHits += templates[sequence].kmers.length;
        //     }



            // else{
            //     delete kmerObject.firstMatches.delete(sequence);
            // }
        // }
        // kmerObject.firstMatches.forEach((hit, sequence) => {
        for (const [sequence, hit] of kmerObject.firstMatches) {
            for (const [queryKmer, kmerCoverage] of kmerQueryMap) {
            // kmerQueryMap.forEach(function (coverage, queryKmer) {
                // filter query kmers that are in the template kmers
                if (hit.kmers.has(queryKmer)) {
                    let template = templates.get(sequence);
                    // let kmerCoverage = kmerQueryMap.get(queryKmer);
                    if (template !== undefined){
                        template.tScore += kmerCoverage;
                        template.uScore += 1;
                        template.kmers.add(queryKmer);
                    }else {
                        templates.set(sequence, {
                            tScore: kmerCoverage,
                            uScore: 1,
                            ...
                            ulength: hit.ulength,
                            species: hit.species,
                            kmers: new Set([queryKmer])
                        });
                    }
                }
            }
            if (templates.has(sequence)) {
                nHits += templates.get(sequence).kmers.size;
            }else{
