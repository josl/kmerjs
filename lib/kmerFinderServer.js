...
function extractKmers() {
    return mongoose.model(kmerObject.collection, kmerMapSchema, kmerObject.collection)
        .aggregate([{
            $match: {
            }
        }])
...
function findKmersMatches(kmerMap, url, collection, progress) {
...
function winnerScoringRecursive(summary, kmerMap, kmerObject) {
    ...
    function recursiveAction() {
        return findKmersMatches(kmerMap, kmerObject.db.url,
