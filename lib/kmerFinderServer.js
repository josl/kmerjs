...
/**
 * [winnerScoringRecursive description]
 * @param  {[type]} hitCounter         [description]
 * @param  {[type]} finishedProcessing [description]
 * @param  {[type]} summary            [description]
 * @param  {[type]} kmerMap            [description]
 * @param  {[type]} kmerObject         [description]
 * @param  {[type]} kmerResults        [description]
 * @return {[type]}                    [description]
 */
function winnerScoringRecursive(hitCounter, notFound, summary, kmerMap, kmerObject, kmerResults) {
    // let minScore = 0;
    let maxhits = 100;
    return new Promise(function (resolve, reject) {
        if (notFound && hitCounter < maxhits){
            // console.log('Query size: ', kmerMap.size);
            findMatchesMongo(kmerMap, kmerObject.db.url, kmerObject.collection, 1)
                .then(function (results) {
                    ...
                    if (winner && kmerObject.evalue.cmp(winner.get('probability')) >= 0){
                        hitCounter += 1;
                        // console.log(hitCounter, ' Best!! ', winner.get('template'));
                ...
                .then(function (matchReads) {
                    if (matchReads){
                        // console.log('lololo');
                        // console.log('Matches found!', matchReads.reads.length, typeof(matchReads.reads));
                        // remove all kmers in best hit from kmerMap
                        matchReads.reads.forEach(function(kmer){
                            kmerMap.delete(kmer);
                        });
                        return winnerScoringRecursive(
                            hitCounter, notFound,
                            summary, kmerMap, kmerObject, kmerResults
                        );
                    }else{
                        notFound = false;
                        console.log('lalala');
...
function winnerScoringRecursive2(summary, kmerMap, kmerObject) {
    let hitCounter = 0;
    let notFound = true;
    let maxHits = 100;
    let kmerResults = [];
    ...
    var loop = function() {
        console.log('Looping... ', hitCounter);
        if (!(notFound && hitCounter < maxHits)){
            // console.log('resolving', kmerResults.length);
            return kmerResults;
        }else{
...
function winnerScoring(kmerObject, kmerMap) {
    return mongoose.model('Summary', kmerSummarySchema, 'Summary')
            ...
            .then(function (summary) {
                console.log(summary);
                return winnerScoringRecursive2(
                    summary, kmerMap,
                    kmerObject
                );
                // return winnerScoringRecursive(
                //     1, true, summary, kmerMap,
                //     kmerObject
                // );
            })
            // .then(function(kmerResults){
            //     console.log(kmerResults.length);
            //     console.log('let\'s see some results');
            //     return kmerResults.sort(sortKmerMatches);
            // })
            .catch(function (error) {
                // Receives first rejection among the Promises
                throw new Error(error);
